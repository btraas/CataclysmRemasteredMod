function Start_mcv(CustomGroup, playerIndex, shipID) 
	print('start_mcv')
	rand = random(1, 2)
	
	if(rand == 1) then
		ship = "Hgn_Smt_mcv_ioncannonfrigate"
	else
		ship = "Hgn_Smt_mcv_supportfrigate"
	end
	
	print('mimicking '..ship)
	
	SobGroup_CreateIfNotExist("mimic_temp_mcv")
	SobGroup_FillShipsByType("mimic_temp_mcv", CustomGroup, "Hgn_Smt_mcv")
	
	SobGroup_AvoidanceIgnore(CustomGroup, "mimic_temp_mcv")
	...
	while(1) do
		...
		if(Wait_End(timer)) then
			SobGroup_Despawn("mimic_temp_mcv")
			return
		end
	end
end
...
function Do_mcv(CustomGroup, playerIndex, shipID)  
...
function Finish_mcv(CustomGroup, playerIndex, shipID)	
...
function Create_mcv(CustomGroup, playerIndex, shipID)
	print('create_mcv')
	SobGroup_SetSwitchOwnerFlag(CustomGroup, 0)
	SobGroup_CreateIfNotExist("mimic_temp_mcv")
	SobGroup_CreateIfNotExist("mimc_temp_mcv1")
	SobGroup_CreateIfNotExist("mimic_temp_mcv2")
	
end

function Update_mcv(CustomGroup, playerIndex, shipID)
...
function Destroy_mcv(CustomGroup, playerIndex, shipID)
