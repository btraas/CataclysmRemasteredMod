...
function ChatterMisc(shipnm,parm2,code)
	...
	if (code==CD_MissileIncoming and shiptype==Capital) then -- only if the victim is a Capital ship
		...
		if(playSomtaawStatus("STATUS_MISSILEINCOMING", shipnm)) then
			return
		end
		if (parm2=="Hgn_TorpedoFrigate" or parm2=="Hgn_TorpedoFrigateElite" or parm2=="Hgn_Destroyer" or parm2=="Hgn_Smt_destroyer" or parm2=="Hgn_Smt_dreadnought") then
	...
	elseif(code==CD_DefenseFieldFound) then
		if(playSomtaawStatus("STATUS_DEFENSEFIELDFOUND", shipnm)) then
			return
		end
	...
	elseif (code==CD_EnteredInhibitorVolume) then
		if(playSomtaawStatus("STATUS_HSINHIB_DETECTED", shipnm)) then
			return
		end
...
function ChatterMiscHighPriority(shipnm,parm2,code)
	...
	if(code==CD_CapturedAtUnitCap) then
		if(playSomtaawStatus("STATUS_CAPTURE_CAPFULL", shipnm)) then
			return
		end
	...
	--	elseif (shipnm=="Hgn_Smt_Worker") then
	--		playSpeechActor("shipsounds/Hgn_Smt_Worker/STATUS_CAPTURE_CAPFULL_1", raceHelper(), 0, Frequency_Command )
 		else
	...
	elseif(code==CD_CaptureHalfway) then
		if(playSomtaawStatus("STATUS_CAPTURE_50PERCENT", shipnm)) then
			return
		end
		if (shipnm=="Hgn_MarineFrigate") then
	...
	elseif(code==CD_CaptureStarted) then
		if(playSomtaawStatus("STATUS_CAPTURE_BEGIN", shipnm)) then
			return
		end
		if (shipnm=="Hgn_MarineFrigate") then
	...
	elseif(code==CD_EnemyCaptureStarted) then
		if(playSomtaawStatus("STATUS_ENEMYCAPTURE_BEGIN", shipnm)) then
			return
		end
	...
	elseif(code==CD_ShipArrived and playSomtaawStatus("STATUS_ARRIVED", shipnm)) then
		return
	...
	elseif(code==CD_ShipEmergingFromHyperpace) then
		if(playSomtaawStatus("STATUS_ENEMY_HS_EMERGING", shipnm)) then
			return
		end
		playSpeechActor("STATUS_HyperspaceUnitEmerging_1",raceHelper(),0, Frequency_Command)
	elseif(code==CD_ShipSpotted and playSomtaawStatus("STATUS_ENEMY_SPOTTED", shipnm)) then
			return
...
attackMap["hgn_smt_acolyte"] = {"STATUS_Fighter_Under_Attack", NameCapPilot,NumCapPilots}
attackMap["hgn_smt_acolytee"] = {"STATUS_Fighter_Under_Attack", NameCapPilot,NumCapPilots}
attackMap["hgn_smt_acv"] = {"STATUS_Corvette_Under_Attack", NameSupportPilot,NumSupportPilots}
attackMap["hgn_smt_acve"] = {"STATUS_Corvette_Under_Attack",  NameCapPilot,NumCapPilots}
attackMap["hgn_smt_dreadnought"] = {"STATUS_Battlecruiser_Attack",  NameCapPilot,NumCapPilots}
attackMap["hgn_smt_destroyer"] = {"STATUS_Destroyer_Under_Attack", NameCapPilot,NumCapPilots}
attackMap["hgn_smt_carrier"] = {"STATUS_Carrier_Under_Attack", NameCapPilot,NumCapPilots}
attackMap["hgn_smt_clee_san"] = {"STATUS_Research_Under_Attack", NameCapPilot,NumCapPilots}
attackMap["hgn_smt_hivefrigate"] = {"STATUS_Frigate_Under_Attack", NameCapPilot,NumCapPilots}
attackMap["hgn_smt_mimic"] = {"STATUS_Fighter_Under_Attack", NameCapPilot,NumCapPilots}
attackMap["hgn_smt_multibeamfrigate"] = {"STATUS_Frigate_Under_Attack", NameCapPilot,NumCapPilots}
attackMap["hgn_smt_processor"] = {"STATUS_Processor_Under_Attack", NameCapPilot,NumCapPilots}
attackMap["hgn_smt_rammingfrigate"] = {"STATUS_Frigate_Under_Attack", NameCapPilot,NumCapPilots}
attackMap["hgn_smt_scaffold"] = {"STATUS_Shipyard_Under_Attack", NameCapPilot,NumCapPilots}
attackMap["hgn_smt_scout"] = {"STATUS_Fighter_Under_Attack", NameCapPilot,NumCapPilots}
attackMap["hgn_smt_sentinel"] = {"STATUS_Fighter_Under_Attack", NameCapPilot,NumCapPilots}
attackMap["hgn_smt_seraph"] = {"STATUS_Battlecruiser_Under_Attack", NameCapPilot,NumCapPilots}
attackMap["hgn_smt_superacolyte"] = {"STATUS_Fighter_Under_Attack", NameCapPilot,NumCapPilots}
attackMap["hgn_smt_Worker"] = {"STATUS_Worker_Under_Attack", NameCapPilot,NumCapPilots}
...
attackMapKuun = {}
attackMapKuun["Hgn_Smt_destroyer"] = "STATUS_Destoyer_Under_Attack_1"
attackMapKuun["Hgn_Smt_Carrier"] = "STATUS_Carrier_Under_Attack_1"
attackMapKuun["Hgn_smt_carrier"] = "STATUS_Carrier_Under_Attack_1"
attackMapKuun["Hgn_Smt_dreadnought"] = "STATUS_Battlecruiser_Under_Attack_1"
attackMapKuun["Hgn_Smt_ms"] = "STATUS_Mothership_Under_Attack_1"
attackMapKuun["Hgn_smt_ms"] = "STATUS_Mothership_Under_Attack_1"
attackMapKuun["Hgn_smt_ms_sp"] = "STATUS_Mothership_Under_Attack_1"
attackMapKuun["Hgn_Smt_Seraph"] = "STATUS_Battlecruiser_Under_Attack_1"
attackMapKuun["Hgn_Smt_hivefrigate"] = "STATUS_frigate_under_attack_1"
attackMapKuun["Hgn_Smt_RammingFrigate"] = "STATUS_frigate_under_attack_1"
attackMapKuun["Hgn_Smt_multibeamfrigate"] = "STATUS_frigate_under_attack_1"
attackMapKuun["Hgn_Smt_ACV"] = "STATUS_corvette_under_attack_1"
attackMapKuun["Hgn_Smt_ACVE"] = "STATUS_corvette_under_attack_1"
attackMapKuun["Hgn_Smt_Mimic"] = "STATUS_fighter_under_attack_1"
attackMapKuun["Hgn_Smt_SuperAcolyte"] = "STATUS_fighter_under_attack_1"
attackMapKuun["Hgn_Smt_AcolyteE"] = "STATUS_fighter_under_attack_1"
attackMapKuun["Hgn_Smt_Acolyte"] = "STATUS_fighter_under_attack_1"
attackMapKuun["Hgn_Smt_scout"] = "STATUS_fighter_under_attack_1"
attackMapKuun["Hgn_Smt_SuperAcolyte"] = "STATUS_fighter_under_attack_1"
attackMapKuun["Hgn_Smt_Processor"] = "STATUS_processor_under_attack_1"
attackMapKuun["Hgn_Smt_Worker"] = "STATUS_Worker_under_attack_1"
attackMapKuun["Hgn_smt_worker"] = "STATUS_Worker_under_attack_1"
attackMapKuun["Hgn_Smt_Clee_San"] = "STATUS_research_under_attack_1"
attackMapKuun["Hgn_Smt_SuperAcolyte"] = "STATUS_fighter_under_attack_1"
...
-- didn't include subsystem repaired/damaged/destroyed
-- build, research available, complete place in kuunlan folder ie STATUS_BuildCorvetteAvaliable_1
...
if (strfind (shipname, "bst_") ~= nil or strfind (shipname, "Bst_") ~= nil) then	
	...
	if(status) then
		print('Playing somtaaw status: '..status..' for ship: '..shipname)
	else
		status = ''
	end
	
	ship = shipname
	if(statusFiles[shipname]) then		
		if(statusFiles[shipname][status] and statusFiles[shipname][status] > 0) then -- If command is set for this ship
			shipname = shipname
		else
			if(statusFiles[shipname]['sameas'] and statusFiles[shipname]['sameas'] ~= '') then -- If sameas is set
				shipname = statusFiles[shipname]['sameas']
			end
		end
	else
		shipname = 'shared'
	end
	if(statusFiles[shipname][status] and statusFiles[shipname][status] > 0) then	-- If ship or sameas has command set
		shipname = shipname
	else
		shipname = 'shared'
	end
	...
	path = statusFiles[shipname]['folder'] .. status
	files = statusFiles[shipname][status]
	
	if(files == nil or files < 1) then
		return
	end
	rand = random(1, files)
	FILENAME = path .. "_" .. rand
	print('playing speech: '..raceHelper() .. "/" .. FILENAME..' for ship: '..ship)
    playSpeechActor(FILENAME, raceHelper(), 0, Frequency_Command )
	return 1
end
...
function StatusUnderAttack(shipnm, enemy, attackcode)
	...
	if(playSomtaawStatus("STATUS_UNDERATTACK", shipnm, enemy)) then
		return
	end
...
function StatusRetaliating(retaliatingShipName, currentTactics)
	...
	if(currentTactics == AggressiveTactics) then
		status = "STATUS_RETALIATE_AGGRESSIVE"
	elseif(currentTactics == DefensiveTactics) then
		status = "STATUS_RETALIATE_DEFENSIVE"
	elseif(currentTactics == PassiveTactics) then
		status = "STATUS_RETALIATE_PASSIVE"
	end
	
	if(playSomtaawStatus(stauts, retaliatingShipName)) then
		return
	end
...
function StatusUnderFireNeedOrders(underFireShipName, targetShipName, groupNumber)
	if(groupNumber == -1) then
		if(playSomtaawStatus("STATUS_UNDERATTACK_NEEDOREDERS", underFireShipName, targetShipName)) then
			return
		end
...
function StatusCaptureComplete(shipnm, targetnm, cameradistance)
	...
	if(playSomtaawStatus("STATUS_CAPTURE_SUCCESS", shipnm, targetnm)) then
		return
	end
...
function StatusInMineField(underFireShipName, groupNumber)
	if(playSomtaawStatus("STATUS_INMINEFIELD", underFireShipName)) then
		return
	end
	if(groupNumber == -1) then
...
function StatusAttackComplete(shipName, groupnum)
	...
	if (groupnum==-1) then
		if(playSomtaawStatus("STATUS_ATTACKCOMPLETE", shipName)) then
			return
		end
...
function StatusRepairs(shipName, targetName, repairStatus)
	...
	-- can't find this event in new speech
		if(playSomtaawStatus("STATUS_STARTEDREPAIRS", shipName)) then
			return
		end
	...
	elseif(repairStatus == FinishedRepairs) then
		if(playSomtaawStatus("STATUS_COMPLETEDREPAIRS", shipName)) then
			return
		end
...
function StatusShipDestroyed(shipName, code)
	...
	if(playSomtaawStatus("STATUS_DESTROYED", shipName)) then
			return
	end
...
function StatusShipDocked(shipName, dockedWithName)
	...
	if(playSomtaawStatus("STATUS_DOCKED", shipName)) then
			return
	end
...
function StatusTakingLightDamage(shipName, healthpercentage)
	...
	if(playSomtaawStatus("STATUS_LIGHTDAMAGE", shipName)) then
			return
	end
...
function StatusTakingHeavyDamage(shipName)
	...
	if(playSomtaawStatus("STATUS_HEAVYDAMAGE", shipName)) then
			return
	end
...
function StatusDamaged(shipName)
	...
	if(playSomtaawStatus("STATUS_DAMAGED", shipName)) then
			return
	end
...
function StatusResources(shipname, actionType)
	...
	if (actionType==1) then
		stat = "STATUS_RESOURCES_EXHAUSTED_MOVINGON"
	elseif (actionType==2) then
		stat = "STATUS_RESOURCES_ENEMIES_DETECTED"
	elseif (actionType==3) then
		stat = "STATUS_RESOURCES_EXHAUSTED_IDLE"
	elseif (actionType==4) then
		stat = "STATUS_RESOURCES_NOMORERUS"
	elseif (actionType==5) then
		stat = "STATUS_RESOURCES_TOOMANYCOLLECTORS"
	elseif (actionType==6) then
		stat = "STATUS_RESOURCES_IDLE"
	end
	
	if(playSomtaawStatus(stat, shipname)) then
			return
	end
...
function StatusCriticallyDamaged(shipName)
	...
	if(playSomtaawStatus("STATUS_CRITICALDAMAGE", shipName)) then
			return
	end
...
function StatusBattleMomentum(momentum)
	...
	if(momentum > 5.0) then
		stat = "STATUS_BATTLE_ADVANTAGE"
	elseif(momentum > 4.0) then
		stat = "STATUS_BATTLE_WINNING"
	elseif(momentum > 2.0) then
		stat = "STATUS_BATTLE_POSITIVE"
	elseif(momentum > 0.5) then -- Brayden/Yelow13 changed to 0.5 from 0.125
		stat = "STATUS_BATTLE_FAIR"
	elseif(momentum < 0.125) then
		stat = "STATUS_BATTLE_NEGATIVE"
	elseif(momentum < 0.25) then
		stat = "STATUS_BATTLE_LOSING"
	elseif(momentum < 0.5) then
		stat = "STATUS_BATTLE_DISADVANTAGE"
	end
	
	if(playSomtaawStatus(stat, "")) then -- No ship field, but will use shared
			return
	end
...
function BattleChatMisc(ssname, parm2, code)
	...
	stat = ''
	if(code==1) then
		stat = "CHATTER_HITFROMBEHIND"
	elseif(code==2) then
		stat = "CHATTER_HITFROMTOPBOTTOM"
	elseif(code==3) then
		stat = "CHATTER_ONEMANSTANDING"
	end
	
	if(playSomtaawStatus(stat, ssname)) then
			return
	end
...
function StatusHyperspaceExit(shipName)
	...
	if(playSomtaawStatus("STATUS_HS_EXIT", shipName)) then
			return
	end
